...
const PortfolioItem = ({ imageUrl, title, description, link, github, tags = [] }: Project) => (
  ...
  >
    <div className="relative w-full h-96" >
      ...
      />
      {/* Base overlay that's always visible */}
      <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/50 to-black/30 opacity-70" />
      {/* Additional overlay on hover */}
      <div className="absolute inset-0 bg-gradient-to-t from-[#3c559a]/90 to-black/50 opacity-0 group-hover:opacity-90 transition-opacity duration-300" />
    </div>
    ...
    <div className="absolute bottom-0 p-8 w-full transform translate-y-8 group-hover:translate-y-0 transition-transform duration-300">
      <div className="bg-black/40 backdrop-blur-lg rounded-xl p-6 text-white">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {tags.map((tag, index) => (
            <span
              key={index}
              className="px-3 py-1 text-xs rounded-full bg-[#3c559a]/30 text-white"
            >
              {tag}
            </span>
          ))}
